...
class DBManager:
    ...
    def __init__(
        ...
        self.doc_sk = metadata.get_sort_key()
    ...
    def delete_md_from_db(self, existing_db_item):
        ...
        if existing_db_item is None:
            raise ValueError(
                "Document not found in DynamoDB when attempting to delete."
            )
    ...
    def get_md_status(self) -> bool:
        ...
        if "Item" not in response:
            raise ValueError(
                "Document not found in DynamoDB when attempting to get status."
            )
        item = response["Item"]
    ...
    def change_md_status(self, existing_db_item):
        ...
        if existing_db_item is None:
            raise ValueError(
                "Attempted to change status of document not found in DynamoDB."
            )
        if constants.DBField.PUBLISHED not in existing_db_item:
            raise ValueError("Publication status field not found on DynamoDB item.")
        existing_db_item[constants.DBField.PUBLISHED] = not existing_db_item[
            constants.DBField.PUBLISHED
        ]
    ...
    def _get_db_item(self):
        ...
        try:
            response = self.table.get_item(
                Key={PK_FIELD: self.doc_pk, SK_FIELD: self.doc_sk}
            )
    ...
    def _create_db_items(
        self, s3_path: str, existing_db_item=None, publish: bool = False
    ):
        """Creates DynamoDB items for the document."""
        db_items = []
